...
testGrowthTry <- function(object, species = NULL, t_max = 1, dt = 0.01, t_save = 0.01) {
  ...
  get_growth_at_t <- function(t) {
    ...
    # Interpolate growth rate across size classes [FIX]
  ...
  # checker <- currently WRONG
#  manual_list <- list(NULL)
#  dw_list <- list(NULL)
#  dws <- sim@params@dw_full
  
  
#  for (inde in seq_along(time_steps)){
#    ts <- dt
    
    #approx g = dw/dt
    #num <- (N_vals[inde,,] - N_vals[inde-1,,]) / ts
    
    
    #using dw exactly
#    dws <- unlist(dws)
#    other_num <- diff(dws) / 0.01
    
    
    
    
#    #manual_list <- append(manual_list, list(num))
 #   dw_list <- append(dw_list, list(other_num))
